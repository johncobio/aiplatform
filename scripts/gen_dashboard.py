...
panels = [
    ...
    ts("Requests / s at ingress by HTTP status", [(f"sum(aiplatform:http_requests:rate5m{SEL}) by (status)", "{{status}}")],
       "reqps", 0, 6, colors={"200": GREEN, "500": RED, "503": ORANGE, "429": ORANGE}, stack=True,
    ...
    ts("Engine-reported latency P50 / P95 / P99", [(qn(50), "p50"), (qn(95), "p95"), (qn(99), "p99")],
       "s", 0, 14, colors={"p50": BLUE_L, "p95": BLUE_M, "p99": BLUE_D}, fill=0,
    ...
    ts("Requests / s reported by engine, by status", [(f"sum(aiplatform:llm_requests:rate5m{SEL}) by (status)", "{{status}}")],
       "reqps", 12, 14, colors={"ok": GREEN, "error": RED, "rejected": ORANGE}, stack=True,
       desc="Engine-native request accounting where available."),
    row("Inference", 22),
    ts("Tokens / s", [(tok, "completion tokens/s"), (ptok, "prompt tokens/s")], "short", 0, 23,
       colors={"completion tokens/s": BLUE_M, "prompt tokens/s": GRAY}, fill=0),
    ts("Per-request generation speed (P50)", [(tps50, "tok/s per request")], "short", 12, 23, w=6,
    ...
    ts("Pending requests (queued + in-flight)", [(f"sum(aiplatform:llm_pending_requests{SEL})", "pending"), (f"sum(aiplatform:llm_pending_requests{SEL}) / clamp_min(sum({ready}), 1)", "per pod (HPA signal)")],
       "short", 18, 23, w=6, colors={"pending": ORANGE, "per pod (HPA signal)": BLUE_M}, fill=0),
    row("Capacity", 31),
    ts("Replicas", [(f"sum({ready})", "ready"), (f"sum({desired})", "desired"), (f"sum({hpa})", "hpa target")],
       "short", 0, 32, w=8, colors={"ready": GREEN, "desired": GRAY, "hpa target": PURPLE}, fill=0),
    ts("CPU (cores)", [(cpu, "used"), (cpu_lim, "limit")], "short", 8, 32, w=8,
       colors={"used": BLUE_M, "limit": GRAY}, fill=0),
    ts("Memory (working set)", [(mem, "used"), (mem_lim, "limit")], "bytes", 16, 32, w=8,
       colors={"used": BLUE_M, "limit": GRAY}, fill=0),
    row("Model and cost (estimates)", 40),
    stat("Model load time (builtin)", f"max(llm_model_load_seconds{SEL})", "s", 0, 41, w=4, decimals=1),
    stat("Est. infra cost / hour", cost_hour, "currencyUSD", 4, 41, w=5, decimals=4,
    ...
    stat("Est. cost per 1k requests", cost_1k, "currencyUSD", 9, 41, w=5, decimals=4,
         desc="Estimated hourly cost divided by requests per hour (1h window), ×1000."),
    stat("Model", f"count by (model) (up{SEL})", "none", 14, 41, w=5, decimals=0),
    stat("Engine", f"count by (engine) (up{SEL})", "none", 19, 41, w=5, decimals=0),
]
